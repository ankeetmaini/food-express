# How To Find Friends Realtime with Pusher and Google Maps

In this tutorial we'll create an awesome app to find the realtime location of our friends! We'll not only be able to find their location but also see them go places in realtime. It so happens that you plan an outing and everyone's not at the venue at the same time. Worry not! you can use this small app to see which of your friends might get late to your party and who is fibbing when they say that they have already left!

This post demonstrates how easy it is to use the realtime capabilities of Pusher and how simple it is to integrate with Google Maps JavaScript APIs to create a real-world tracking utility, be it an e-commerce parcel tracking or tracking your food delivery or a FEDEX package!

You can try out the app by [Finding your friends](https://track-friends.herokuapp.com) by getting their usernames!

To give you a sneak peak this is how our app will look at the end of this post.

![](https://dl.dropboxusercontent.com/s/hz3janocdsskuyw/how-to-find-friends-realtime-all.gif)

## Step 0: Setting up our app

- Start off by creating an `index.html` with this code snippet.
- Our humble [`index.html`](https://github.com/ankeetmaini/track-friends/blob/master/index.html) doesn't do much except include a `meta` tag in the `head` section so that our app looks crisp and works great in mobile browsers as well.
...
- Once you get the key, copy it, and include their JS file in the `index.html`. Also add the tag for `app.js` which will contain our app's code.
...
```
![](https://dl.dropboxusercontent.com/s/lnz8c06vidvw0si/how-to-find-friends-realtime-opening.jpeg)
...
- With this code, we get the location and center the map. See we're already into delivering a personalized experience!
...
- It's important to capture a *username* to identify users on the map
...
```
- Take a look at the latest version of our [app.js](https://github.com/ankeetmaini/track-friends/blob/59b48ad8a51b8b42f89100ca3446ce78a6bca6db/src/app.js) ![](https://dl.dropboxusercontent.com/s/lzxumc1dfs9a69h/how-to-find-friends-realtime-login.jpeg)
 
## Step 3: Set up tracking logic, send events on location change

- To **track locations of our friends** we'll use [Pusher's real time capabilities](https://pusher.com/). We'll trigger events whenever we change our location and also at the same time listen for the location change events of our friends.
...
```
- To handle the case when the user isn't moving, we add a `setInterval` to keep sending the last captured location. This means that our friends can track our last location, instead of disappearing from the map!
...
```
![](https://dl.dropboxusercontent.com/s/xgi7jed6fkj00qg/how-to-find-friends-realtime-matt.jpeg)
...
```
- So every time you add a username, `addFriend` function would get called.
...
- So that if the person is moving, you can see the marker moving on the map in realtime. ZOMG! I KNOW! :D
 
## Step 5: Finding multiple friends, all at once

- Now that you've learned how to find a friend in the above step, we can do it for multiple people
...
![](https://dl.dropboxusercontent.com/s/aj6wewmja86qebb/how-to-find-friends-realtime-dorothy.jpeg)
...
``` bash
  git clone git@github.com:ankeetmaini/find-friends.git
  cd find-friends
  npm install
  npm start
```
...
- Code is hosted on [GitHub](https://github.com/ankeetmaini/find-friends)
